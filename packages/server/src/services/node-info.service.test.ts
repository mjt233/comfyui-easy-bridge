...
describe('generateNodeClassDts / generateBuildDts', () => {
  it('generates dts with class union, field keys, and dynamic signatures', () => {
    ...
    expect(full).toContain('addNode<K extends ComfyClassType>(nodeId: string, classType: K, inputs?: Partial<ComfyNodeInputs[K]> & Record<string, unknown>, title?: string): ComfyNode;');
